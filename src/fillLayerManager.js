...
export class FillLayerManager {
  constructor(map) {
    ...
    this.map = map;
    this.layers = new Map();
    // The base URL for all data requests
  ...
  async addLayer(id, options, beforeId) {
    ...
    // Destructure all required options for URL construction and layer styling
    const { 
        model, 
        date, 
        run, 
    ...
    // Validate that all necessary components for the URL are provided
    if (!model || !date || !run || forecastHour === undefined || !variable || smoothing === undefined || !colormap) {
      throw new Error('Missing required options. You must provide model, date, run, forecastHour, variable, smoothing, and colormap.');
    }

    // Look up the grid configuration from the provided model name
    const gridConfig = COORDINATE_CONFIGS[model];
    if (!gridConfig) {
        throw new Error(`Model "${model}" not found in coordinate configurations.`);
    }
    const grid = gridConfig.grid_params;

    // Construct the data URL from the provided options
    ...
    try {
      ...
      if (!response.ok) throw new Error(`HTTP error! Status: ${response.status} for URL: ${dataUrl}`);
      const { data: b64Data, encoding } = await response.json();
      const compressedData = Uint8Array.from(atob(b64Data), c => c.charCodeAt(0));
      
      // Post the data and encoding info to the worker
      ...
      const handleWorkerMessage = (e) => {
        ...
        if (e.data.success) {
          ...
          // Use the grid dimensions (nx, ny) from the looked-up config
